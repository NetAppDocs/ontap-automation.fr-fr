...
---
= Découvrez comment migrer vers l'API REST ONTAP
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
SnapDiff v1 et v2 sont basées sur le framework de gestion ONTAPI (ZAPI) et présentent certaines limitations d'utilisation avec certaines versions d'ONTAP. L'API SnapDiff v3 n'utilise pas ONTAPI et est compatible avec ONTAP 9.8 et les versions ultérieures. La disponibilité de l'API SnapDiff pour les différentes versions d'ONTAP est définie comme suit :

* ONTAP 9.7 et versions antérieures : v1 et v2 (ONTAPI uniquement)
* ONTAP 9.8 et versions ultérieures : v1, v2 et v3. Certaines limitations s’appliquent ; consultez la  https://kb.netapp.com/on-prem/ontap/DP/SnapDiff/SnapDiff-KBs/FAQ_SnapDiff_Support_in_ONTAP["FAQ : SnapDiff Support dans ONTAP"^] pour plus de détails sur la prise en charge de SnapDiff selon la version d’ONTAP.


Vous devez vérifier que vos applications de sauvegarde prennent en charge SnapDiff et être conscient de toute limitation d'interopérabilité en fonction de la version d'ONTAP que vous utilisez. Consultez également le https://library.netapp.com/ecm/ecm_download_file/ECMLP2492508["Notes de version de ONTAP"^] pour plus d'informations.
...
* https://kb.netapp.com/on-prem/ontap/DP/SnapDiff/SnapDiff-KBs/FAQ_SnapDiff_Support_in_ONTAP["FAQ : SnapDiff Support dans ONTAP"^]
