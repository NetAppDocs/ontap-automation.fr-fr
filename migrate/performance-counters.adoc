...
summary: 'Le gestionnaire de compteur ONTAP tient à jour des informations complètes sur les performances de chaque système ONTAP. Il exporte ces données sous la forme d"un ensemble de compteurs de performances que vous pouvez utiliser pour évaluer les performances de votre système ONTAP et vous aider à atteindre vos objectifs de performances.' 
---
...
Vous pouvez accéder à la https://docs.netapp.com/us-en/ontap-pcmap/["Mappage du compteur de performances ONTAP"^] documentation en ligne. Un sélecteur de version permet également d'accéder aux versions précédentes de la documentation basée sur la version ONTAP.
