...
Le NetApp.ONTAP PowerShell Toolkit enrichit votre environnement PowerShell local grâce à un module comprenant plus de 2 400 cmdlets. Il vous permet de développer rapidement du code pour votre hôte Windows afin d'automatiser les déploiements ONTAP. Pour plus d'informations, consultez link:../pstk/learn-about-pstk.html["Découvrez le NetApp ONTAP PowerShell Toolkit"].
