...
.Stratégies IPsec
Vous pouvez utiliser cet ensemble d'appels d'API pour gérer les stratégies en vigueur pour un déploiement IPSec. Ce type de ressource est nouveau avec ONTAP 9.8. Elles ont été mises à jour avec la version 9.19 pour récupérer les stratégies utilisées pour le réseau du cluster.

.Associations de sécurité IPsec
Vous pouvez utiliser cet ensemble d'appels d'API pour gérer les associations de sécurité en vigueur pour un déploiement IPSec. Ce type de ressource est nouveau avec ONTAP 9.8. Elles ont été mises à jour avec la version 9.19 pour récupérer les associations de sécurité utilisées pour le réseau du cluster.
...
L'authentification web (WebAuthn) est une norme web permettant d'authentifier les utilisateurs de manière sécurisée grâce à la cryptographie à clé publique. Avec ONTAP, elle prend en charge l'administration de MFA résistantes au phishing via System Manager et l'API REST ONTAP. Cette fonctionnalité a été ajoutée avec ONTAP 9.16 et mise à jour avec ONTAP 9.19.

.Qui suis-je
Un point de terminaison permet d'afficher les informations de l'utilisateur authentifié, notamment son nom d'utilisateur, son rôle et ses autorisations. Ce type de ressource a été introduit avec ONTAP 9.19.
