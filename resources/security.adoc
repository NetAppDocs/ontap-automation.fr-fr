...
L'authentification Web (WebAuthn) est une norme Web permettant d'authentifier les utilisateurs de manière sécurisée grâce à la cryptographie à clé publique. Avec ONTAP, elle prend en charge l'administration de MFA résistantes au phishing via System Manager et l'API REST ONTAP. Cette fonctionnalité a été ajoutée avec ONTAP 9.16 et mise à jour avec ONTAP 9.19.1.

.Qui suis-je
Un point de terminaison permet d'afficher les informations de l'utilisateur authentifié, notamment son nom d'utilisateur, son rôle et ses autorisations. Ce type de ressource a été introduit avec ONTAP 9.19.1.
