...
La version ONTAP 9.19.1 comprend plusieurs mises à jour qui continuent d'améliorer et d'affiner l'API REST ONTAP.

Sécurité:: Une nouvelle `whoami` fonctionnalité a été ajoutée pour récupérer le nom d'utilisateur, le rôle et les informations d'autorisations de l'utilisateur connecté. La capacité d'authentification web (WebAuthn) a également été améliorée. IPsec a été mis à jour pour récupérer les stratégies et les associations de sécurité utilisées avec le réseau du cluster.
SAN:: L'administration de la cartographie LUN a été mise à jour avec une capacité de mise à jour améliorée.
