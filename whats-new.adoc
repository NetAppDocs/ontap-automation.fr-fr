...
La version ONTAP 9.18.1 continue d'étendre les capacités de l'API REST ONTAP avec onze nouveaux appels d'API. Les améliorations portent principalement sur la sécurité, mais comprennent également une amélioration relative à l'administration du stockage.

Sécurité:: Deux améliorations majeures en matière de sécurité ont été introduites avec l'API REST ONTAP 9.18.1. Vous pouvez utiliser les nouveaux points de terminaison de l'API réseau du cluster pour afficher et mettre à jour la configuration de sécurité du réseau du cluster. Une prise en charge robuste de la gestion des certificats est incluse pour le réseau du cluster. De plus, vous pouvez afficher et mettre à jour la configuration de sécurité du réseau HA pour le trafic NVLog.
Stockage:: NetApp FlexCache est un cache persistant d'un volume d'origine. Il prend en charge la distribution en éventail, ce qui permet de créer plusieurs FlexCache à partir d'un seul volume d'origine. L'API REST inclut la prise en charge de la récupération de l'état de la connexion entre les instances de cache et les volumes d'origine.
...
Médiateurs de cluster:: Vous pouvez envoyer une requête ping au service cloud NetApp Console à l'aide de l'API REST ONTAP . Vous pouvez également modifier la configuration d'un médiateur spécifique avec la méthode PATCH sur un point de terminaison existant.
